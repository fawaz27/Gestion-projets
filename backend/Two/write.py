...
def GetChapters1(baseUrl,url,limit):
    ...
    result = []
    ChapterLinks = soup.find_all('ul',{'class':'ul-list5'})
    ...
    for chapterslink in ChapterLinks:
        ...
        for link in ChapterLinks1:
            if i == limit:
                break
            #print('Downloading',link,'at',baseUrl+link.get('href'))
            # f = open(link.get('title')+".html",'w+')
            # f.write(str(GetChapterContent(baseUrl,link.get('href'))))
            # print(str((GetChapterContent(baseUrl,link.get('href') )) ))
    ...
    #ChapterLinks1 = soup.find('ul',{'class':'list-chapter'}).find_all('a')
...
def WriteinFiles(baseUrl,url,limit):
    ...
    path=Path("src/public/novels/"+name)
    path.mkdir(parents=True, exist_ok=True)
    #print(type(novel))
    saveImg(image,"src/public/novels/"+name)
    for l in novel:
        content=l['content']
        ...
        for k in content:
            s+=str(k)+'\n \n'
        f = open("src/public/novels/"+name+"/"+str(chaptertitre).replace("/","-")+".html",'w+')
        f.write(s)
        f.close()
        s=''
    #print(name)
